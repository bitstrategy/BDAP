    ...
    var active = {
      ...
      , notice: function () {
          ...
          layer.open({
            ...
            , content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">亲！您确定要退出系统吗？/(ㄒoㄒ)/~~</div>'
            , success: function (layero) {
...
function addIframe(cur) {
    ...
    if (!isHas) {
        ...
        var iframe = "<iframe class='body-iframe' name='iframe" + m + "' width='100%' height='99%' src='" + url + "' frameborder='0' data-id='" + h + "' seamless></iframe>";
    ...
    for (var i = 0; i < menu.length; i++) {
        ...
        try {
            if (item.isHeader == "1") {
                ...
                if (item.childMenus == "") {
                    str = "<li><a href='" + item.url + "' data-id='" + item.id + "'><i class='icon-font'>" + item.icon + "</i><span>" + item.name + "</span></a></li>";
                    $(parent).append(str);
                } else {
...
$(function () {
    ...
    $.ajax({
        type: "POST",
        url: "../JSONData/MenuList",
        dataType: "json",
        data: {},
        contentType: "application/json; charset=utf-8",
        success: function (ret) {
            initMenu(ret, $(".side-menu"));
    ...
    //       {
    //           "id": "3", "name": "高层管理驾驶舱", "parentId": "1", "url": "", "icon": "&#xe604;", "order": "1", "isHeader": "0", "childMenus": [
    //              { "id": "4", "name": "支出分析", "parentId": "3", "url": "http://127.0.0.1/Analyzer2361SSO/dashboard/designer.aspx?action=open&tabindex=2&reportid=1266545672&folderid=-2112223416&token=WUFBQUFnQUFBQXdWWVRzdW05VzZKZEl6bG1PdXlCUHF5N1MvdDlYbXREdTJIbWpvWm5xelQ2Zkp6akxYZVdjeA&rt=1&tb=1&userid=biadmin&password=biadmin", "icon": "", "order": "1", "isHeader": "0", "childMenus": "" },
    //              { "id": "5", "name": "分类管理", "parentId": "3", "url": "test2.html", "icon": "", "order": "1", "isHeader": "0", "childMenus": "" }
    //           ]
    //       },
    //       {
    //           "id": "6", "name": "商品分析", "parentId": "1", "url": "", "icon": "&#xe602;", "order": "1", "isHeader": "0", "childMenus": [
    ...
    //},
    //			{
    //			    "id": "2", "name": "零售分析", "parentId": "0", "url": "", "icon": "", "order": "2", "isHeader": "1", "childMenus": [
